...
export const OneProductMainComponent = (props: OneProductMainComponentInterface) => {
    ...
    return (
        <section>
            <section className={one_product_scss.root + ' round_section'}
                ...
                <footer className={one_product_scss.info}>
                    <button className={one_product_scss.more}
                            onClick={() => {
                                Cookies.set('id', props.oneProduct.id.toString())
                                router.push(Path.PRODUCTS + '/' + props.oneProduct.id)
                            }}>
                        Больше информации
                    </button>
